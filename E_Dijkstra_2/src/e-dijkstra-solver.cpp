...
int e_dijkstra_solver(char const* filepath)
{
    ...
    int minRequiredRecharges = std::numeric_limits<int>::max();
    int maxRequiredRecharges = 0;

    std::vector<std::vector<std::pair<int, int>>> graph;
    graph.resize(totalTown);
    for (int i = 0; i < totalPaths; ++i)
    {
        int townOne, townTwo, distance;
        fileStream >> townOne >> townTwo >> distance;

        graph[townOne].emplace_back(townTwo, distance);
        graph[townTwo].emplace_back(townOne, distance);

        if (minRequiredRecharges > distance)
        {
            minRequiredRecharges = distance;
        }

        maxRequiredRecharges += distance;
    }
    fileStream.close();

    while (minRequiredRecharges < maxRequiredRecharges) 
    {
        // MidPoint for binary search, a potential for mid point
        const int potentialRechargeValue_MidPoint = minRequiredRecharges + (maxRequiredRecharges - minRequiredRecharges) / 2;

        if (e_dijkstra(filepath, potentialRechargeValue_MidPoint))
        {
            maxRequiredRecharges = potentialRechargeValue_MidPoint;
        }
        else 
        {
            minRequiredRecharges = potentialRechargeValue_MidPoint + 1;
        }
    }

    return minRequiredRecharges;
}