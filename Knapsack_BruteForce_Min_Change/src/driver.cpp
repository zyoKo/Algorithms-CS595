...
void test2() { test_knapsack("./src/items0", 100); } // set delay to 1/10 s. There are 16-1 iterations, so total time should be about 1.5+epsilon if implemented using min change
void test3() { test_knapsack("./src/items1", 0); }
void test4() { test_knapsack("./src/items2", 0); }
void test5() { test_knapsack("./src/items3", 0); }
void test6() { test_knapsack("./src/items4", 0); } // 7m 58.754s !!!!!!!
...
int main()
{
	// Run test manually
	unsigned x;
	std::cout << "Input number to run test(0 - 6): ";
	std::cin >> x;
	switch (x)
	{
	case 0:
	...
	default:
		std::cout << "Can't run tests, invalid test number!\n";
	}
...
int main(int argc, char** argv)
{
	...
	catch (char const* str) { std::cout << "Exception Raised in main: " << str << std::endl; }
